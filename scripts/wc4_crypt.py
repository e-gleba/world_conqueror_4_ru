...
v3.0.0: parse/query/edit subcommands, JSON pretty-print, regex sub,
...
class C:
    ...
    def disable(cls) -> None:
        for attr in ("RST", "BLD", "DIM", "RED", "GRN", "YLW", "CYN", "MAG", "WHT"):
            setattr(cls, attr, "")
...
def _is_text(data: bytes) -> bool:
    if len(data) < 2:
        return False
    try:
        t = data.decode("utf-8", errors="strict")
        return t.lstrip()[0:1] in ("{", "[", "<", '"', "'")
...
def detect_header(raw: bytes) -> HeaderKind | None:
    ...
    if _has_easy(raw):
        candidates += [HeaderKind.EASY_MD5_SIZE, HeaderKind.EASY_MD5]
    candidates += [HeaderKind.MD5_SIZE, HeaderKind.MD5, HeaderKind.RAW]
        ...
        try:
            ...
            if _is_text(pt):
                ...
                if hdr in (HeaderKind.EASY_MD5_SIZE, HeaderKind.EASY_MD5):
                    md5_ok = raw[12:28] == hashlib.md5(pt).digest()
                elif hdr in (HeaderKind.MD5_SIZE, HeaderKind.MD5):
                    md5_ok = raw[:16] == hashlib.md5(pt).digest()
...
def json_get(data: bytes, path: str) -> Any:
    """Dot-notation path: 'units.0.name' => obj["units"][0]["name"].
    Supports wildcard: 'units.*.name' => [u["name"] for u in obj["units"]].
    """
    obj = json.loads(data.decode("utf-8"))
    return _resolve_path(obj, path.split("."))
...
def _resolve_path(obj: Any, parts: list[str]) -> Any:
    ...
    if isinstance(obj, list):
        return _resolve_path(obj[int(head)], rest)
    if isinstance(obj, dict):
        return _resolve_path(obj[head], rest)
    raise KeyError(f"cannot index {type(obj).__name__} with '{head}'")


def json_set(data: bytes, path: str, value: Any) -> bytes:
    obj = json.loads(data.decode("utf-8"))
...
def regex_sub(data: bytes, pattern: str, replacement: str, count: int = 0) -> tuple[bytes, int]:
    text = data.decode("utf-8")
    result, n = re.subn(pattern, replacement, text, count=count)
...
def _decrypt_file(src: Path, dst: Path, *, pretty: bool = False) -> tuple[bool, str]:
    ...
    if res.md5_ok is True:
        md5_tag = f" {_dim('md5:ok')}"
    elif res.md5_ok is False:
        md5_tag = f" {C.YLW}md5:mismatch{C.RST}"
    pretty_tag = f" {_dim('pretty')}" if pretty and out != res.plaintext else ""
    return True, f"{src.name} {_dim('=>')} {dst.name}  {_dim(_size(len(out)))}  [{res.header.name}]{md5_tag}{pretty_tag}"


def _encrypt_file(src: Path, dst: Path, header: HeaderKind = HeaderKind.MD5_SIZE) -> tuple[bool, str]:
    pt  = src.read_bytes()
    enc = encrypt(pt, header)
    dst.write_bytes(enc)
    return True, f"{src.name} {_dim('=>')} {dst.name}  {_dim(_size(len(pt)))} {_dim('=>')} {_dim(_size(len(enc)))}  [{header.name}]"


def _encrypt_file_auto(src: Path, dst: Path, ref: Path | None) -> tuple[bool, str]:
    header = HeaderKind.MD5_SIZE
    if ref and ref.is_file():
        raw = ref.read_bytes()
        detected = detect_header(raw)
        if detected is not None:
            header = detected
    return _encrypt_file(src, dst, header)
...
def cmd_roundtrip(args: argparse.Namespace) -> int:
    ...
    if is_encrypted:
        ...
        hdr = HeaderKind(args.header) if hasattr(args, "header") and args.header is not None else HeaderKind.MD5_SIZE
        enc = encrypt(raw, hdr)
...
def _to_plaintext(raw: bytes, label: str) -> bytes | None:
    ...
    res = decrypt(raw)
    if res:
        _info(label, f"no EASY header, decrypted [{res.header.name}] => {_size(len(res.plaintext))}")
        return res.plaintext
    _info(label, f"raw {_size(len(raw))}")
    return raw


def _header_diff(label_a: str, a: bytes, label_b: str, b: bytes, hdr: HeaderKind) -> None:
    if hdr in (HeaderKind.EASY_MD5_SIZE, HeaderKind.EASY_MD5):
        def _p(d: bytes) -> dict[str, Any]:
            ver, plen = struct.unpack("<II", d[4:12])
            md5 = d[12:28].hex()
            osz = struct.unpack("<I", d[28:32])[0] if hdr == HeaderKind.EASY_MD5_SIZE and len(d) >= 32 else None
            return {"ver": ver, "payload_len": plen, "md5": md5, "orig_size": osz}
    elif hdr in (HeaderKind.MD5_SIZE, HeaderKind.MD5):
        def _p(d: bytes) -> dict[str, Any]:
            md5 = d[:16].hex()
            osz = struct.unpack("<I", d[16:20])[0] if hdr == HeaderKind.MD5_SIZE and len(d) >= 20 else None
            return {"md5": md5, "orig_size": osz}
    else:
        return
...
def _first_diff(a: bytes, b: bytes, label_a: str, label_b: str, max_diffs: int = 5) -> None:
    minlen = min(len(a), len(b))
    shown = 0
    for i in range(minlen):
        if a[i] != b[i]:
            ctx_a = a[max(0, i - 8):i + 8]
            ctx_b = b[max(0, i - 8):i + 8]
            _fail("diff", f"offset 0x{i:08x}: {label_a}=0x{a[i]:02x} {label_b}=0x{b[i]:02x}")
            _info("", f"{label_a}: {ctx_a.hex(' ')}")
            _info("", f"{label_b}: {ctx_b.hex(' ')}")
            shown += 1
            if shown >= max_diffs:
                remaining = sum(1 for j in range(i + 1, minlen) if a[j] != b[j])
                if remaining:
                    _warn(f"{remaining} more differing bytes")
                break
    if len(a) != len(b):
        _fail("size", f"{label_a}={len(a)} {label_b}={len(b)} delta={len(b) - len(a):+d}")
...
def cmd_decrypt(args: argparse.Namespace) -> int:
    ...
    if src.is_dir():
        return _batch(src, Path(args.output or "decrypted"), decrypt_mode=True,
                      jobs=args.jobs, pretty=pretty, glob=getattr(args, "glob", None))
    dst = Path(args.output) if args.output else Path(src.stem + ".decrypted" + src.suffix)
...
def cmd_encrypt(args: argparse.Namespace) -> int:
    ...
    if ref_path and ref_path.is_file():
        detected = detect_header(ref_path.read_bytes())
        if detected is not None:
            header = detected
            _info("auto", f"detected header format from ref: {header.name}")

    if src.is_dir():
        return _batch(src, Path(args.output or "encrypted"), decrypt_mode=False,
                      jobs=args.jobs, header=header, glob=getattr(args, "glob", None))
    dst = Path(args.output) if args.output else Path(src.stem + ".encrypted" + src.suffix)
...
def cmd_info(args: argparse.Namespace) -> int:
    ...
    if _has_easy(raw):
        ...
        if len(raw) >= 32:
            orig_sz = struct.unpack("<I", raw[28:32])[0]
            _info("orig_size", f"{orig_sz} ({_size(orig_sz)})")
    else:
        _info("magic", "no EASY header")

    res = decrypt(raw)
    if res:
        _ok("decrypt", f"ok  header={res.header.name}  plain={_size(len(res.plaintext))}")
        kind = detect_content(res.plaintext)
        _info("content", kind.name)
        preview = res.plaintext[:200].decode("utf-8", errors="replace").replace("\n", "\\n")
        _info("preview", _dim(preview))
        if res.md5_ok is True:
            _ok("md5", "verified")
        elif res.md5_ok is False:
            _fail("md5", "MISMATCH")
            _info("", f"header:   {raw[12:28].hex() if _has_easy(raw) else raw[:16].hex()}")
            _info("", f"computed: {hashlib.md5(res.plaintext).hexdigest()}")
    else:
...
def cmd_verify(args: argparse.Namespace) -> int:
    ...
    for f in files:
        if not f.is_file():
            continue
        if pat and not fnmatch.fnmatch(f.name, pat):
            continue
        raw = f.read_bytes()
        res = decrypt(raw)
        if res and res.md5_ok is True:
            _ok("pass", f.name)
            ok += 1
        elif res and res.md5_ok is None:
            _info("skip", f"{f.name}  {_dim('no md5 in header')}")
        elif res and res.md5_ok is False:
...
def cmd_parse(args: argparse.Namespace) -> int:
    ...
    if kind == ContentKind.JSON:
        try:
            obj = json.loads(pt)
            formatted = json.dumps(obj, indent=args.indent, ensure_ascii=False,
                                   sort_keys=getattr(args, "sort_keys", False))
            top = f"object keys={len(obj)}" if isinstance(obj, dict) else f"array len={len(obj)}"
...
def cmd_edit(args: argparse.Namespace) -> int:
    ...
    if args.regex:
        for spec in args.regex:
            ...
            if len(parts) < 2:
                _fail("regex", f"bad format: expected {sep}pattern{sep}replacement{sep} got '{spec}'")
                return 1
    ...
    if args.json_set:
        try:
            for assignment in args.json_set:
                path_str, _, value_str = assignment.partition("=")
                if not _:
    ...
    if re_encrypt and orig_hdr is not None:
        enc = encrypt(pt, orig_hdr)
        dst.write_bytes(enc)
        _ok("write", f"{dst.name}  encrypted [{orig_hdr.name}]  {_size(len(enc))}")
    elif re_encrypt:
        enc = encrypt(pt, HeaderKind.MD5_SIZE)
        dst.write_bytes(enc)
        _ok("write", f"{dst.name}  encrypted [MD5_SIZE]  {_size(len(enc))}")
    else:
        dst.write_bytes(pt)
    ...
    for f in files:
        ...
        if hits:
            for lineno, line in hits:
                highlighted = re.sub(
                    f"({re.escape(args.pattern) if not args.regex_mode else args.pattern})",
                    lambda m: f"{C.RED}{C.BLD}{m.group()}{C.RST}",
                    line,
                )
                _ok(f"{f.name}", f"{_dim(str(lineno) + ':')} {highlighted}")
            total_hits += len(hits)

    print(f"\n  {_dim(f'{total_hits} match(es)')}\n")
    return 0 if total_hits > 0 else 1


# -- batch -----------------------------------------------------------------

def _batch(src_dir: Path, out_dir: Path, *, decrypt_mode: bool, jobs: int,
           header: HeaderKind = HeaderKind.MD5_SIZE, pretty: bool = False,
           glob: str | None = None) -> int:
    ...
    for f in files:
        dst = out_dir / f.name
        if decrypt_mode:
            success, msg = _decrypt_file(f, dst, pretty=pretty)
        else:
            success, msg = _encrypt_file(f, dst, header)
        if success:
            _ok(op, msg)
            ok += 1
        else:
...
def _summary(ok: int, fail: int, elapsed: float | None = None) -> None:
    ...
    print(f"\n  {C.BLD}{color}{ok}/{total} succeeded{C.RST}", end="")
    if fail:
...
""",
    ...
    s.add_argument("--pretty", action="store_true", help="pretty-print JSON output")
    s.add_argument("--glob", metavar="<pattern>", help="filter filenames in batch mode (e.g. '*.json')")
    ...
    s.add_argument("-o", "--output", metavar="<path>")
    s.add_argument("-j", "--jobs", type=int, default=1, metavar="N")
    s.add_argument("--header", metavar="<fmt>", default=None,
                   help=f"header format: {_HEADER_NAMES} (default: auto-detect from --ref, else MD5_SIZE)")
    s.add_argument("--ref", metavar="<file>",
                   help="reference encrypted file to auto-detect header format")
    s.add_argument("--glob", metavar="<pattern>", help="filter filenames in batch mode")
    ...
    s.add_argument("input", metavar="<path>")
    s.add_argument("--glob", metavar="<pattern>", help="filter filenames")

    # -- roundtrip
    s = sub.add_parser("roundtrip", help="verify encrypt<->decrypt produces identical plaintext")
    s.add_argument("input", metavar="<file>")
    ...
    s.add_argument("input", metavar="<file>")
    s.add_argument("-o", "--output", metavar="<path>", help="write formatted output to file")
    s.add_argument("--indent", type=int, default=2, metavar="N", help="JSON indent (default: 2)")
    s.add_argument("--sort-keys", action="store_true", dest="sort_keys", help="sort JSON keys")
    s.add_argument("--keys", action="store_true", help="list top-level keys (JSON objects)")

    # -- query
    s = sub.add_parser("query", help="decrypt + extract JSON value by dot-path")
    s.add_argument("input", metavar="<file>")
    s.add_argument("path", metavar="<json.path>",
                   help="dot-notation path, e.g. 'units.0.name' or 'units.*.attack'")

    # -- edit
    s = sub.add_parser("edit", help="regex sub / JSON set-del on decrypted content")
    s.add_argument("input", metavar="<file>")
    s.add_argument("-o", "--output", metavar="<path>", help="output file (default: overwrite input)")
    s.add_argument("--regex", "-r", action="append", metavar="'/pat/repl/'",
                   help="sed-style regex substitution (first char = separator)")
    s.add_argument("--set", "-s", action="append", dest="json_set", metavar="'path=value'",
                   help="set JSON value by dot-path (e.g. 'units.0.hp=200')")
    s.add_argument("--del", "-d", action="append", dest="json_del", metavar="'path'",
                   help="delete JSON key/index by dot-path")
    s.add_argument("--encrypt", "-e", action="store_true",
                   help="re-encrypt output using original header format")

    # -- grep
    s = sub.add_parser("grep", help="search decrypted content with regex")
    s.add_argument("input", metavar="<path>", help="file or directory")
    s.add_argument("-p", "--pattern", required=True, metavar="<regex>", help="search pattern")
    s.add_argument("--glob", metavar="<pattern>", help="filter filenames (e.g. '*.json')")
    s.add_argument("--regex-mode", action="store_true", dest="regex_mode",
                   help="treat -p as full regex (default: literal for highlighting)")
