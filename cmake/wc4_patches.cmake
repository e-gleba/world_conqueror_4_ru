...
function(wc4_discover_patches)
  ...
  foreach(manifest IN LISTS manifests)
    cmake_path(GET manifest PARENT_PATH patch_dir)
    cmake_path(GET patch_dir FILENAME name)
  ...
  set(wc4_patches_known "${known}" PARENT_SCOPE)
...
function(wc4_register_patch_tests name tree)
  ...
  foreach(test_src IN LISTS test_srcs)
    cmake_path(GET test_src STEM test_stem)
...
function(wc4_add_variant name)
  ...
  add_custom_command(
    ...
    COMMAND "${CMAKE_COMMAND}" -E copy_directory "${decompiled_dir}" "${tree}"
    COMMAND "${CMAKE_COMMAND}" -E touch "${stage_stamp}"
    DEPENDS "${stamp_decompiled}" ${decompiled_files} ${payload_deps}
  ...
  foreach(p IN LISTS active)
    ...
    ExternalProject_Add_StepDependencies(${ep} configure "${stage_stamp}"
                                         ${payloads_${p}})
  ...
  add_custom_command(
    ...
    COMMAND
      ...
      "-DMODE=encrypt" "-DDIR=${tree}/${assets_data_rel}" -P "${crypt_script}"
    COMMAND
    ...
    DEPENDS patch-${name} "${stage_stamp}" ${payload_deps} "${wc4_crypt}"
            "${crypt_script}" "${apktool_jar}" "${signer_jar}"
    COMMENT
